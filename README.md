...
2.  **Frontend (`frontend/.env`):**
    * Currently, the frontend directly uses `http://localhost:4000` for API calls and sockets. If you want to make this configurable, create `frontend/.env` and add:
        ```dotenv
        VITE_API_BASE_URL=http://localhost:4000/api
        VITE_SOCKET_URL=http://localhost:4000
        ```
    * You would then need to update `api.ts` and `AuthContext.tsx` (or `useSocket.ts`) to use `import.meta.env.VITE_API_BASE_URL` and `import.meta.env.VITE_SOCKET_URL` respectively. *(Note: We haven't implemented this configuration step yet)*.
