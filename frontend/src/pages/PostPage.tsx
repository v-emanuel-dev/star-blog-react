...
import { getPostById, deletePost, getCommentsForPost, addComment, editComment, deleteComment } from '../services/api';
...
const PostPage: FC = () => {
    ...
    const handleDeletePost = async () => {
         if (!id || !post) return;
         const confirmDelete = window.confirm(`Are you sure you want to delete the post "${post.title}"?`);
         if (confirmDelete) {
             setIsDeletingPost(true); setError(null);
             try {
                 await deletePost(id);
                 navigate('/', { replace: true });
             } catch (err) {
                  setError(err instanceof Error ? `Error deleting post: ${err.message}` : 'Unknown error.');
    ...
    const handleCommentSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setNewComment('');
        } catch (err) { setCommentsError(err instanceof Error ? `Failed to add comment: ${err.message}` : 'Unknown error.'); }
    ...
    const handleSaveEdit = async (commentId: number) => {
        ...
        setIsSavingEdit(true); setCommentsError(null);
        try {
            const result = await editComment(commentId, { content: editedCommentContent.trim() });
            setComments(prev => prev.map(c => c.id === commentId ? result.comment : c));
            handleCancelEdit();
        } catch (err) { setCommentsError(err instanceof Error ? `Failed to update comment: ${err.message}` : 'Unknown error.'); }
    ...
    const handleDeleteComment = async (commentId: number) => {
        ...
        if (confirmDelete) {
            setDeletingCommentId(commentId); setCommentsError(null);
            try {
                await deleteComment(commentId);
                setComments(prev => prev.filter(c => c.id !== commentId));
            } catch (err) { setCommentsError(err instanceof Error ? `Failed to delete comment: ${err.message}` : 'Unknown error.'); }
    ...
    if (error && !post) { return ( <div className="px-4 sm:px-6 lg:px-8 py-4 text-center"><Alert message={error} type="error" title="Error!" onClose={() => setError(null)} /><Link to="/" className="text-blue-500 hover:underline mt-4 inline-block">Back to homepage</Link></div> ); }
            ...
            <article className="bg-white rounded-lg shadow-md p-6 relative pb-10 mb-8">
                 {loggedInUser && (
                    <div className="absolute top-4 right-4 flex space-x-2 z-10">
                        <Link to={`/edit-post/${post.id}`} className="bg-yellow-500 hover:bg-yellow-600 text-white text-xs font-semibold py-1 px-3 rounded transition duration-150 ease-in-out">Edit Post</Link>
                        <button onClick={handleDeletePost} disabled={isDeletingPost} className={`bg-red-600 hover:bg-red-700 text-white text-xs font-semibold py-1 px-3 rounded transition duration-150 ease-in-out ${isDeletingPost ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    ...
                    {commentsLoading ? ( <div className="flex justify-center py-6"><Spinner size="md"/></div> )
                     ...
                     : comments.length === 0 ? ( <p className="text-gray-500 text-center py-4">Be the first to comment!</p> )
                      : ( comments.map((comment) => {
                            ...
                            const userCanModify = loggedInUser && loggedInUser.id === comment.user?.id;

                            return (
                                <div key={comment.id} className={`flex space-x-3 border-b border-gray-200 pb-4 last:border-b-0 ${isDeletingThisComment ? 'opacity-50' : ''}`}>
                                    ...
                                    <div className="flex-1">
                                        <div className="flex justify-between items-start mb-1">
                                            <div>
                                                 ...
                                                 <span className="text-xs text-gray-500 ml-2">{formatDisplayDateTime(comment.createdAt)}</span>
                                            ...
                                            {userCanModify && !isEditingThisComment && (
                                                <div className="flex space-x-2 flex-shrink-0 ml-2">
                                                     <button onClick={() => handleEditClick(comment)} title="Edit comment" className="text-gray-400 hover:text-blue-600 disabled:opacity-50" disabled={isSavingEdit || !!deletingCommentId}>
                                                         <FontAwesomeIcon icon={faEdit} className="h-3 w-3"/>
                                                     </button>
                                                      <button onClick={() => handleDeleteComment(comment.id)} title="Delete comment" className="text-gray-400 hover:text-red-600 disabled:opacity-50" disabled={isSavingEdit || !!deletingCommentId || isDeletingThisComment}>
                                                           {isDeletingThisComment ? <Spinner size="sm"/> : <FontAwesomeIcon icon={faTrashAlt} className="h-3 w-3"/>}
                                                      </button>
                                                </div>
                                            )}
                                        </div>
                                        {isEditingThisComment ? (
                                            <div className="mt-2">
                                                <textarea
                                                    ...
                                                    onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setEditedCommentContent(e.target.value)}
                                                    rows={3}
                                                ...
                                                <div className="mt-2 flex justify-end space-x-2">
                                                    <button onClick={() => handleSaveEdit(comment.id)} disabled={isSavingEdit || editedCommentContent === comment.content} className={`inline-flex items-center px-3 py-1 border border-transparent text-xs font-medium rounded shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed`}>
                                                         {isSavingEdit ? <Spinner size="sm" color="text-white" className="mr-1"/> : <FontAwesomeIcon icon={faSave} className="h-3 w-3 mr-1"/>} Save
                                                    </button>
                                                     <button onClick={handleCancelEdit} disabled={isSavingEdit} className={`inline-flex items-center px-3 py-1 border border-gray-300 text-xs font-medium rounded shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50`}>
                                                        <FontAwesomeIcon icon={faTimes} className="h-3 w-3 mr-1"/> Cancel
                                                    </button>
                                                </div>
                                            </div>
                                        ) : (
