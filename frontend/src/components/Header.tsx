    ...
    <nav className="bg-gray-800 shadow-md">
        ...
        <div className="relative flex h-16 items-center justify-between">
          ...
          <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
            ...
            <div className="hidden sm:ml-6 sm:block">
              <div className="flex space-x-4 items-center"> {/* Adicionado items-center para alinhar links com o logo maior */}
                <Link to="/" className={getLinkClasses('/')} aria-current={location.pathname === '/' ? 'page' : undefined}>
                  Home
                </Link>
                <Link to="/about" className={getLinkClasses('/about')} aria-current={location.pathname === '/about' ? 'page' : undefined}>
                  Sobre
                </Link>
              </div>
            </div>
      ...
      <div className={`${isMobileMenuOpen ? 'block' : 'hidden'} sm:hidden`} id="mobile-menu">
        <div className="space-y-1 px-2 pt-2 pb-3">
          <Link to="/" className={getLinkClasses('/', true)} aria-current={location.pathname === '/' ? 'page' : undefined} onClick={() => setIsMobileMenuOpen(false)}>
            Home
          </Link>
          <Link to="/about" className={getLinkClasses('/about', true)} aria-current={location.pathname === '/about' ? 'page' : undefined} onClick={() => setIsMobileMenuOpen(false)}>
            Sobre
          </Link>
        </div>
      </div>
