...
interface AlertProps {
  ...
  title?: string;
  onClose?: () => void; // Optional callback function for closing
}
...
const Alert: FC<AlertProps> = ({
  ...
  let titleText = title;
  let iconColor = 'text-red-500'; // Color for the 'X' icon
  ...
  return (
    <div
      className={`${bgColor} ${borderColor} ${textColor} border px-4 py-3 rounded relative mb-4`} // Added mb-4 for spacing below alert
      role="alert"
    >
      {titleText && <strong className="font-bold mr-2">{titleText}</strong>}
      <span className="block sm:inline">{message}</span>

      {/* Add Close Button only if onClose function is provided */}
      {onClose && (
        <span
